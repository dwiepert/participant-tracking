...
def archive(files, archive_path):
    ...
    """
    for f in files:
        dst_path = archive_path / Path(f).name
        shutil.move(f, dst_path)

def get_most_recent(pat, archive_path, sep="_",dt_ind=-2, dt_format='%Y%m%d', data_ext=".xlsx"):
    """
...
def generate_parentstudy(reports, out_path, archive_path, load_existing, code='', dt_ind=-2, dt_format='%Y%m%d', data_ext ='.xlsx', sep="_"):
    ...
    if load_existing:
        ...
        r_names= [str(r.name) for r in reports]
        rm_reports = [r for r in in_reports if r.name not in r_names]
        reports = [r for r in reports if r.name not in ir_names]
...
def load_parentstudy(input_path, out_path, load_existing, to_archive, archive_path, code=''):
...
def pop_parentstudy(df, input_path, output_path,load_existing, to_archive, report_archive, code=''):
    #temp = df.loc[(pd.isnull(df['ParentStudy']) | pd.isnull(df['Site']))]
    #mrns = temp['MRN'].values


    parentstudies = load_parentstudy(input_path, output_path, load_existing, to_archive, report_archive, code=code)
...
def main():
    ...
    parser.add_argument("--decrypt_code",default='', help='specify code for decrypting file as a string')
    ...
    ## MC_RSH_AI
    idfd, idfd_date = get_most_recent(pat=args.input_path / 'MC_RSH_AI_Speech_Identified*.xlsx', archive_path=args.input_archive, sep="_",
                                      dt_ind=[-2,-1], dt_format='%Y%m%d%H%M', data_ext='.xlsx')

    inr, inr_date = get_most_recent(pat=args.input_path / 'MC_RSH_AI_Speech_Interested*.xlsx', archive_path=args.input_archive, sep="_",
                                      dt_ind=[-2,-1], dt_format='%Y%m%d%H%M', data_ext='.xlsx')

    n_int, n_int_date = get_most_recent(pat=args.input_path / 'MC_RSH_AI_Speech_Not_Interested*.xlsx', archive_path=args.input_archive, sep="_",
                                       dt_ind=[-2,-1], dt_format='%Y%m%d%H%M', data_ext='.xlsx')
    ...
    ## PTRAX
    tl, tl_date = get_most_recent(pat= args.input_path / '[0-9]*trackingLog*.xlsx', archive_path=args.input_archive, sep="_",
    ...
    ## QUALTRICS
    q, q_date = get_most_recent(pat=args.input_path / 'dashboard-export*.xlsx', archive_path=args.input_archive, sep="-",
                                dt_ind=[2,3,5,6,7], dt_format='%H%M%Y%m%d', data_ext='.xlsx')

    q2, q_date2 = get_most_recent(pat= args.input_path / 'qualtrics_tracking*.xlsx', archive_path=args.input_archive, sep="_",
    ...
    master_file, master_date = get_most_recent(pat = args.output_path / 'master_database*.csv', archive_path = args.output_archive, sep="_",
    ...
    master = pop_parentstudy(master, args.input_path, args.output_path, args.load_existing, True, args.input_archive, args.decrypt_code)
